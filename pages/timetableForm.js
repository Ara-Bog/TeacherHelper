...
export default class TimetableForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataForm: this.props.route.params.dataForm,
      clientsData: [],
  ...
  loadDataClient() {
    ...
    db.transaction(tx => {
      {
        this.state.dataForm.Type == 'g'
          ? (sqlRequst = 'SELECT name as label, id as value FROM Groups')
          : (sqlRequst =
              "SELECT name || ' ' || surname as label, id as value FROM Students");
      }
      tx.executeSql(
        sqlRequst,
        [],
  ...
  checkValues() {
    ...
    {
      this.state.dataForm.ID != undefined
        ? this.updateNote()
        : this.updateBase();
    }
  }
  ...
  updateBase() {
    db.transaction(tx => {
      tx.executeSql(
        'INSERT INTO timetable (time, client_id, day, type) VALUES (?,?,?,?)',
        [
          this.state.dataForm.Time,
          this.state.dataForm.Client_id,
          this.state.dataForm.Day,
          this.state.dataForm.Type,
        ],
        () =>
          Alert.alert('Успешно', 'Запись добавленна', [
            {
              text: 'Продолжить',
              style: 'default',
            },
  ...
  updateNote() {
    db.transaction(tx => {
      tx.executeSql(
        'UPDATE timetable SET time = ?, client_id  = ?, day  = ?, type  = ? WHERE id = ?',
        [
          this.state.dataForm.Time,
          this.state.dataForm.Client_id,
          this.state.dataForm.Day,
        ...
        () =>
          Alert.alert('Успешно', 'Запись обновленна', [
            {
              text: 'Продолжить',
              style: 'default',
            },
      ...
      <View style={Styles.container}>
        ...
        <View style={Styles.rowForm}>
          <TouchableOpacity
            ...
            <Text style={Styles.formDataTimeText}>
              Время{' '}
              {this.state.dataForm.Time != undefined
                ? '- ' + this.state.dataForm.Time
                : null}
            </Text>
          </TouchableOpacity>
        ...
        <View style={Styles.rowForm}>
          <TouchableOpacity
            ...
            <Text style={Styles.submitBtnText}>
              {this.state.dataForm.ID != undefined ? 'Сохранить' : 'Создать'}
            </Text>
          </TouchableOpacity>
        </View>
