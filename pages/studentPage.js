      ...
      db.transaction(tx => {
        tx.executeSql(
          ...
          (_, {rows: {_array}}) =>
            this.setState({
              currentSounds:
                _array[0].Sound != null ? JSON.parse(_array[0].Sound) : {},
              currentSymptoms:
                _array[0].Symptoms != null
                  ? JSON.parse(_array[0].Symptoms)
                  : {},
              currentDataClient: _array[0],
              clientAge: Math.round(
                (new Date().getTime() - new Date(_array[0].DateBD)) /
                  (24 * 3600 * 365.25 * 1000),
              ),
              checkedViolations:
                _array[0].Violations != null
                  ? _array[0].Violations.split('/')
                  : [],
              dataClient: _array[0],
            }),
              ...
              <Text
                ...
                this.state.categories.length > 0
                  ? this.state.categories.filter(
                      item =>
                        item.value == this.state.currentDataClient.Categori_id,
                    )[0].label
                  : 'Не выбранно'}
              </Text>
            )}
              ...
              <Text
                ...
                this.state.subGroups.length > 0
                  ? this.state.subGroups.filter(
                      item =>
                        item.value == this.state.currentDataClient.Subgroup_id,
                    )[0].label
                  : 'Не выбранно'}
              </Text>
            )}
              ...
              <Text
                ...
                this.state.diagnosis.length > 0
                  ? this.state.diagnosis.filter(
                      item =>
                        item.value == this.state.currentDataClient.Diagnos_id,
                    )[0].label
                  : 'Не выбранно'}
              </Text>
            )}
                ...
                {this.state.editing
                  ? this.state.violations.map(item => (
                      <Text
                        key={item.value}
                        style={
                          this.state.checkedViolations.includes(
                            item.value.toString(),
                          )
                        ...
                        onPress={() =>
                          this.violationSelected(item.value.toString())
                        }>
                        {item.label}
                      </Text>
                    ))
                  : this.state.violations
                      ...
                      .map(item => (
                        <Text
                          key={item.value}
                          style={Styles.cardStudentElement}>
                          {item.label}
                        </Text>
                      ))}
              </View>
